...
struct hicmp{//structure to save highscore campaign mode
		char name[50];
		int score;
}highcmp[10],cmpprsnt,tempcmp;
...
void filecampread(){//read campaign highscore from file
	int iterator;
	fpcamp=fopen("camphigh.txt","r");
	for(iterator=0;iterator<10;iterator++){
		fscanf(fpcamp,"%s %d ",&highcmp[iterator].name,&highcmp[iterator].score);
	}

	fclose(fpcamp);
}

void filecampwrite(){//write campaign highscore to file
	int iterator;
	fpcamp=fopen("camphigh.txt","w");
	for(iterator=0;iterator<10;iterator++){
		fprintf(fpcamp,"%s %d ",highcmp[iterator].name,highcmp[iterator].score);
	}

	fclose(fpcamp);
}

void camphighchng(int v)
	{//campaign highscore update
		int t,u,w;
		if(v>highcmp[9].score){
			highcmp[9].score=v;
			strcpy(highcmp[9].name,name);
		}
		for(t=0;t<9;t++){
			for(u=t+1;u<10;u++){
				if(highcmp[t].score<highcmp[u].score){
					tempcmp=highcmp[t];
					highcmp[t]=highcmp[u];
					highcmp[u]=tempcmp;
				}
...
void campshow(){//campaign high score show
	iShowBMP(290,scrnht-160,"halloffame.bmp");
	iSetColor(0,0,0);
	iFilledRectangle(100,80,1150-80,340);
	iSetColor(255,255,0);
	iText(440,340+60,"RANK            NAME                           SCORE",GLUT_BITMAP_TIMES_ROMAN_24);
	for(i=0;i<10;i++){
		sprintf(str,"%3d               %-15s                 %5d",i+1,highcmp[i].name,highcmp[i].score);
		iSetColor(255,255,255);
		iText(440,345+80-30*(i+2),str);
	}
	//code for showing campaign highscore here
}
